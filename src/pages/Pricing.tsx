...
const Pricing = () => {
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    // If multiple gateways, show selector; otherwise use the only one
    if (enabledGateways.length > 1) {
      setShowGatewaySelector(true);
    } else {
      await processPayment(planId, enabledGateways[0].name);
    }
  };


  const processPayment = async (planId: string, gatewayName: string) => {
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 relative ml-[70px]">
        ...
        <main className="container mx-auto px-4 py-8 pt-28 pb-16 max-w-6xl">
          ...
          <AnimatePresence>
            {showGatewaySelector && selectedPlan && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                ...
                >
                  ...
                  <div className="space-y-3">
                    {enabledGateways.map((gw) => (
                      <button
                        key={gw.id}
                        onClick={() => processPayment(selectedPlan!, gw.name)}
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {gw.is_test_mode ? '🧪 Test Mode' : 'Live'}
                          </p>
                        </div>
                      </button>
                    ))}
