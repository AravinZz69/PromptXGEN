...
const TemplateGenerator = () => {
  ...
  // Find template config - first try hardcoded ID, then fetch from DB and match by title
  useEffect(() => {
    const loadTemplate = async () => {
      setTemplateLoading(true);
      
      // Try direct ID match first
      const directMatch = templateConfigs.find(t => t.id === templateId);
      if (directMatch) {
        setTemplate(directMatch);
        initFormDefaults(directMatch);
        setTemplateLoading(false);
        return;
      }

      // Fetch from DB and match by title
      try {
        ...
        if (dbTemplate) {
          const title = dbTemplate.title || dbTemplate.name;
          // Try matching by title (case-insensitive)
          const titleMatch = templateConfigs.find(
            t => t.title.toLowerCase() === title?.toLowerCase()
          );
          if (titleMatch) {
            setTemplate(titleMatch);
            initFormDefaults(titleMatch);
          } else {
            // Create a dynamic template from DB data
            const dynamicTemplate: TemplateConfig = {
              id: dbTemplate.id,
              title: title || 'Content Generator',
              description: dbTemplate.description || 'Generate content based on your requirements',
              category: dbTemplate.category || 'General',
              fields: [
                { id: 'topic', label: 'Topic', type: 'text', placeholder: `Enter topic for ${title}`, required: true },
                { id: 'description', label: 'Details', type: 'textarea', placeholder: 'Describe what you need in detail...' },
                { id: 'format', label: 'Output Format', type: 'select', options: ['Detailed', 'Summary', 'Point-wise', 'Step-by-step'] },
                { id: 'gradeLevel', label: 'Grade/Level', type: 'text', placeholder: 'e.g., Class 10, Beginner' },
              ],
            };
            setTemplate(dynamicTemplate);
            initFormDefaults(dynamicTemplate);
          }
        } else {
          setTemplate(defaultTemplate);
          initFormDefaults(defaultTemplate);
        }
      } catch (err) {
    ...
    const initFormDefaults = (tmpl: TemplateConfig) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-card border border-border rounded-2xl p-6">
                ...
                <Button
                  ...
                  className="w-full mt-6 bg-primary hover:bg-primary/90 text-primary-foreground gap-2 h-12"
                >
                  {isGenerating ? (
                    <>
